...
Elec2014 <- read.csv("Elec2014.csv")

elecOutcomes <- data.frame(mcStep = 0, cd2DEM = 72938, cd2REP = 63044, winner = "D")
...
for (b in seq(0, 1, 0.25) ) { 
 for (mcStep in 1:50) {
     ...
     if (b == 1.0) {
         outcome <- filter(Elec2014, prec %in% nodes(congDist2)) %>%
             group_by(party) %>% 
             summarise(newelec = sum(as.integer(votes)))

         winner <- if ( outcome[2, 2] > outcome[1, 2] ) {
                       "R" } else {
                               "D"
                           }
         elecOutcomes <-
